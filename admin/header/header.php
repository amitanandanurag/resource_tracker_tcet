...
while ($row = $result->fetch_assoc()) {
	...
	$name = $row['username'];
  $full_email = $name; 
  $user_name = strstr($full_email, '@', true); // Extracts 'superadmin'
  $display_name = ucwords(str_replace(['.', '_'], ' ', $user_name));
}
...
<head>
  ...
  .sidebar-mini.sidebar-collapse .main-sidebar {
    background-color: #0f172a !important; /* The Deep Navy */
  }
  ...
  .sidebar-mini.sidebar-collapse .sidebar-menu > li > a {
    ...
    text-align: center;
    margin: 5px 5px !important; /* Added margin for that "floating" look */
    border-radius: 12px;
  }

  .sidebar-mini.sidebar-collapse .sidebar-menu > li:hover > a {
    background: #7c3aed !important; /* Purple highlight on hover in mini mode */
    color: #fff !important;
  }
  ...
  /* --- 6. USER PANEL FIX --- */
  .user-panel > .info > a {
    color: #10b981 !important; /* Make the "Online" text pop */
    font-weight: 600;
  }
  ...
  /* THEME GRADIENTS */
  .theme-1 { background: linear-gradient(135deg, #6366f1, #a855f7); }
  .theme-2 { background: linear-gradient(135deg, #3b82f6, #2dd4bf); }
  .theme-3 { background: linear-gradient(135deg, #f43f5e, #fb923c); }
  .theme-4 { background: linear-gradient(135deg, #8b5cf6, #d946ef); }
  .theme-5 { background: linear-gradient(135deg, #1e293b, #475569); }
</style>
	...
	<div class="wrapper">
		...
		<header class="main-header">
			...
			<nav class="navbar navbar-static-top">
              ...
              <ul class="dropdown-menu shadow-lg" style="width: 280px; border-radius: 8px; overflow: hidden; border: none;">
        <!-- User image and info -->
        <li class="user-header" style="background: linear-gradient(135deg, #605ca8 0%, #3c8dbc 100%); height: auto; padding: 25px 15px;">
            <img src="dist/img/user2-160x160.jpg" class="img-circle" alt="User Image" style="border: 3px solid rgba(255,255,255,0.2); box-shadow: 0 4px 10px rgba(0,0,0,0.2);">
            ...
            <span class="label label-default" style="background: rgba(255,255,255,0.2); font-size: 10px; text-transform: uppercase; letter-spacing: 1px;">
                Role: Super Admin
            </span>
        </li>
        ...
        <li class="user-footer" style="background-color: #ffffff; padding: 15px;">
            <div class="pull-left">
                <a href="profile.php" class="btn btn-default btn-flat" style="border-radius: 4px; border: 1px solid #ddd;">
                    <i class="fa fa-user"></i> My Profile
                </a>
            </div>
            <div class="pull-right">
                <a href="../login/logout.php" class="btn btn-danger btn-flat" style="border-radius: 4px; background-color: #dd4b39; border: none; padding: 6px 15px;">
